...
$(document).ready(function() {
    ...
    $(document).on('click', '#AddNewId', function() {
        $('#ToppicActionId').text('Add Over Time Type');
        ClearAddNew();
        $('.SlideTabSearch').hide();
        $('#AreaAddId').show();
    });
    ...
    $(document).on('click', '.EditButton', function() {
        console.log("Edit");
        $('#ToppicActionId').text('Edit Over Time Type');
    ...
    $(document).on('click', '.ViewButton',async function() {
        console.log("View");
        $('#ToppicActionId').text('View Over Time Type');
    ...
    $(document).on('click', '.DeleteButton', function() {
        MessageBox.ConfirmDeletedMessage("", () => {
          ...
          return $.ajax({
            ...
            url: config.apiUrl.base + '/api/hr/overtimetype/Delete/' + id
          }).then((response) => {
            return true;
          }).catch((error) => {
    ...
    $(document).on('click', '.ActiveButton', function () {
        ...
        if (button.hasClass('btn-success')) {
            ...
            $.ajax({
                url: config.apiUrl.base + '/api/hr/overtimetype/activateCondition' + id + '&user_id=' + loginId + '&is_active=false',
                type: 'GET',
                contentType: 'application/json; charset=utf-8',
                data: {
            ...
            $.ajax({
                url: config.apiUrl.base + '/api/hr/overtimetype/activateCondition' + id + '&user_id=' + loginId + '&is_active=true',
                type: 'GET',
                contentType: 'application/json; charset=utf-8',
                data: {
          ...
          Page.mainTableControl = $('#tableShowListId').DataTable({
            ...
            ajax: (data, callback) => {
                $.ajax({
                    ...
                    url: config.apiUrl.base+'/api/hr/overtimetype/GetPaginate',
                    type: 'GET',
                    contentType: 'application/json; charset=utf-8',
                    headers: {"Authorization": "Bearer " + token},
                    beforeSend: () => {
           ...
           columnDefs: [
                ...
                { targets: 1, data: 'overtime_type', ordering: true },
                { targets: 2, data: 'overtime_type_comment', ordering: true },                
                { targets: 3, data: 'overtime_type_status', ordering: true },                
                {
                    targets: 4,
                    data: null,
                    orderable: false,
                    ...
                    render: function (data, type, row) {
                        return `<div class="row"><button type="button" class="btn btn btn-info btn-sm ViewButton col-4" data-id="${row.overtime_type_id}" ><i class="bx bx-list-ul me-1"></i></button> ` +
                            ` <button type="button" class="btn btn-primary btn-sm EditButton col-4" data-id="${row.overtime_type_id}"><i class="bx bx-edit me-1"></i></button> `+
                            ` <button type="button" class="btn btn-danger btn-sm DeleteButton col-4" data-id="${row.overtime_type_id}"><i class="bx bx-trash me-1"></i></button> </div>`;
                    }
                }
            ]
          });
    ...
    function ShowUploadListDataTable(formData)
    {
        ...
        $.ajax({
            url: config.apiUrl.base+'/api/hr/overtimetype/ImportDataExcelFile',
            headers: {"Authorization": "Bearer " + token},
            type: 'POST',
            data: formData,
            processData: false,
    ...
    function getTableSortBy(column){
        switch (column) {
            ...
            case 2:
                return 'overtime_type_comment';
            case 3:
                return 'overtime_type_status';                     
            default:
                return null;
        }
    }
    ...
    async function  AddFromSubmit(formData)
    {
        ...
        if (formData.overtime_type_id == '0') {
            //insert 
            formData.create_by = username;                                
            
            var type = 'POST';
            var url =config.apiUrl.base+'/api/hr/overtimetype/addnew';
        }else{
            //update
            formData.update_by = username;

            type = 'PUT';
            url = config.apiUrl.base+'/api/hr/overtimetype/update';
        }
        
        // เพิ่มข้อมูลสถานะของ checkbox
        var isChecked = $('#overtime_type_status').is(':checked');
        formData.overtime_type_status = isChecked ? 'ACTIVE' : 'INACTIVE';
    ...
    {        
        await $.ajax({
            url: config.apiUrl.base+'/api/hr/overtimetype/get/'+id,
            type: 'GET',
            contentType: 'application/json; charset=utf-8',
            // data: {
            //     id : id
            ...
            success: function(response) {
                ...
                if(response.status == true){
                    ...
                    $('#AddFormId [name=overtime_type_id').val(res.overtime_type_id);
                    $('#AddFormId [name=overtime_type]').val(res.overtime_type);
                    $('#AddFormId [name=overtime_type_comment').val(res.overtime_type_comment);                                        
                    
                    // ตั้งค่า dept_status
                    if (res.overtime_type_status === 'INACTIVE') {
                        $('#overtime_type_status').prop('checked', false); // ตั้งค่าเป็น unchecked
                        $('#label_status').next('.form-check-label').text('In Active'); // เปลี่ยนข้อความ
                    } else {
                        $('#overtime_type_status').prop('checked', true); // ตั้งค่าเป็น checked
