...
$(document).ready(function() {
    ...
    $(document).on('click', '.DeleteButton', function() {
        MessageBox.ConfirmDeletedMessage("", () => {
          ...
          return $.ajax({
            ...
            url: config.apiUrl.base + '/Api/Auth/Role/Delete/' + id
          }).then((response) => {
            return true;
          }).catch((error) => {
    ...
    $(document).on('click', '.ActiveButton', function () {
        ...
        if (button.hasClass('btn-success')) {
            ...
            $.ajax({
                url: config.apiUrl.base + '/Api/Auth/role/activateCondition' + id + '&user_id=' + loginId + '&is_active=false',
                type: 'GET',
                contentType: 'application/json; charset=utf-8',
                data: {
            ...
            $.ajax({
                url: config.apiUrl.base + '/Api/Auth/role/activateCondition' + id + '&user_id=' + loginId + '&is_active=true',
                type: 'GET',
                contentType: 'application/json; charset=utf-8',
                data: {
    ...
    function ShowUploadListDataTable(formData)
    {
        ...
        $.ajax({
            url: config.apiUrl.base+'/api/auth/role/ImportDataExcelFile',
            headers: {"Authorization": "Bearer " + token},
            type: 'POST',
            data: formData,
            processData: false,
    ...
    async function  AddFromSubmit(formData)
    {
        ...
        if (jsonData.id == '0') {
            ...
            var type = 'POST';
            var url =config.apiUrl.base+'/api/auth/role/addnew';
        }else{
            //update
            jsonData.update_by = userId;
            type = 'PUT';
            url = config.apiUrl.base+'/api/auth/role/update';
        }
        await $.ajax({
            url: url,
            type: type,
    ...
    {
        await $.ajax({
            url: config.apiUrl.base+'/api/auth/role/get/'+id,
            type: 'GET',
            contentType: 'application/json; charset=utf-8',
            // data: {
            //     id : id
