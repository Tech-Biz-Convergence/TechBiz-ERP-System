...
$(document).ready(function () {
  ...
  $(document).on('click', '.ActiveButton', function () {
    ...
    if (button.hasClass('btn-primary')) {
      ...
      urlApi = config.apiUrl.base + '/Api/Hr/Holiday/activateCondition/' + id;
      status = "INACTIVE";
  ...
  function ShowMainListDataTable() {
    Page.mainTableControl = $('#tableShowListId').DataTable({
      ...
      columnDefs: [
        ...
        { targets: 1, data: 'holiday_name', ordering: true },
        { targets: 2, data: 'holiday_year', ordering: true },
        {
          ...
          render: function (data, type, row, meta) {
            ...
            return formatDate(row.holiday_day)
          },
          ordering: true
        },
        {
          ...
          render: function (data, type, row) {
            return `<div class="dropdown">
                            ...
                            <a class="dropdown-item ViewButton" href="javascript:void(0);" data-id="${row.holiday_id}" 
                                ><i class="bx bx-list-ul me-2"></i> View</a
                              >
                              <a class="dropdown-item EditButton" href="javascript:void(0);" data-id="${row.holiday_id}" 
                                ><i class="bx bx-edit-alt me-2"></i> Edit</a
                              >
                              <a class="dropdown-item DeleteButton" href="javascript:void(0);" data-id="${row.holiday_id}" 
  ...
  async function AddFromSubmit(formData) {
    ...
    if ($('#AddFormId [name=holiday_status]').prop('checked')) {
      formData.holiday_status = 'ACTIVE'
    } else {
      formData.holiday_status = 'INACTIVE'
    }
      ...
      success: function (response) {
        ...
        if (response.status == true) {
          ...
          if (res.holiday_status === "ACTIVE") {
            ...
            $('#AddFormId [name=holiday_status]').prop('checked', false); // ตั้งค่าให้ checkbox ไม่มีสถานะ checked
          }
