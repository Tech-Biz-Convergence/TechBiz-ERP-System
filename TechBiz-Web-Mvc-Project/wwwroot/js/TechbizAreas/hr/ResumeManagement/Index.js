...
$(document).ready(function () {
  ...
  $(document).on('click', '.ActiveButton', function () {
    ...
    if (button.hasClass('btn-primary')) {
      ...
      urlApi = config.apiUrl.base + '/api/hr/Resume/activateCondition/' + id;
      status = "INACTIVE";
      ...
      columnDefs: [
        {
          ...
          render: function (data, type, row) {
            ...
            var buttonClass = row.resume_status === 'ACTIVE' ? 'btn-primary' : 'btn-outline-secondary';
            // สร้าง HTML ของปุ่ม
            return `<button type="button" class="btn rounded-pill btn-icon ${buttonClass} ActiveButton button-feature " data-id="${row.hr_resume_id}">
                              <span class="tf-icons bx bx-power-off"></span>
                        </button>`;
          }
        },
        { targets: 1, data: 'hr_job_title', ordering: true },
        { targets: 2, data: 'hr_candidate_name', ordering: true },
          ...
          render: function (data, type, row) {
            return `<div class="dropdown">
                            ...
                            <div class="dropdown-menu">
                              <a class="dropdown-item ViewButton" href="javascript:void(0);" data-id="${row.hr_resume_id}"><i class="bx bx-list-ul me-2"></i> View</a>
                              <a class="dropdown-item EditButton" href="javascript:void(0);" data-id="${row.hr_resume_id}"><i class="bx bx-edit-alt me-2"></i> Edit</a>
                              <a class="dropdown-item DeleteButton" href="javascript:void(0);" data-id="${row.hr_resume_id}"><i class="bx bx-trash me-2"></i> Delete</a>
                            </div>
                    </div>`;
          }
        }
  ...
  async function AddFromSubmit(formData) {
    ...
    if ($('#AddFormId [name=resume_status]').prop('checked')) {
      formData.resume_status = 'ACTIVE'
    } else {
      formData.resume_status = 'INACTIVE'
    }
    ...
    await $.ajax({
      ...
      success: function (response) {
        ...
        if (response.status == true) {
          ...
          if (res.resume_status === "ACTIVE") {
            $('#AddFormId [name=resume_status]').prop('checked', true); // ตั้งค่าให้ checkbox มีสถานะ checked
          } else {
            $('#AddFormId [name=resume_status]').prop('checked', false); // ตั้งค่าให้ checkbox ไม่มีสถานะ checked
          }
  ...
  function ClearAddNew() {
    ...
    $('#AddFormId [name=hr_resume_id]').val('0');
