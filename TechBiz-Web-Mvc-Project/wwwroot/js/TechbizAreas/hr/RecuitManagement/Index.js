...
$(document).ready(function () {
  ...
  $("#AddFormId").validate({
    ...
    messages: {
      hr_candidate_id: {
        required: icon + " Please select candidate.",        
      },
      hr_job_id: {
        required: icon + " Please select job.",        
      },
      pay_amount: {
        required: icon + " Please enter pay amount.",
  ...
  $(document).on('click', '.ActiveButton', function () {
    ...
    if (button.hasClass('btn-primary')) {
      ...
      urlApi = config.apiUrl.base + '/api/hr/recuit/activateCondition/' + id;
      status = "INACTIVE";
      ...
      columnDefs: [
        {
          ...
          render: function (data, type, row) {
            ...
            var buttonClass = row.recuit_stage_status === 'ACTIVE' ? 'btn-primary' : 'btn-outline-secondary';
            // สร้าง HTML ของปุ่ม
            return `<button type="button" class="btn rounded-pill btn-icon ${buttonClass} ActiveButton button-feature " data-id="${row.recuit_stage_id}">
          ...
          render: function (data, type, row) {
            return `<div class="dropdown">
                            ...
                            <div class="dropdown-menu">
                              <a class="dropdown-item ViewButton" href="javascript:void(0);" data-id="${row.recuit_stage_id}"><i class="bx bx-list-ul me-2"></i> View</a>
                              <a class="dropdown-item EditButton" href="javascript:void(0);" data-id="${row.recuit_stage_id}"><i class="bx bx-edit-alt me-2"></i> Edit</a>
                              <a class="dropdown-item DeleteButton" href="javascript:void(0);" data-id="${row.recuit_stage_id}"><i class="bx bx-trash me-2"></i> Delete</a>
                            </div>
                    </div>`;
          }
        }
  ...
  async function AddFromSubmit(formData) {
    ...
    if ($('#AddFormId [name=recuit_stage_status]').prop('checked')) {
      formData.recuit_stage_status = 'ACTIVE'
    } else {
      formData.recuit_stage_status = 'INACTIVE'
    }
    ...
    await $.ajax({
      ...
      success: function (response) {
        ...
        if (response.status == true) {
          ...
          if (res.recuit_stage_status === "ACTIVE") {
            $('#AddFormId [name=recuit_stage_status]').prop('checked', true); // ตั้งค่าให้ checkbox มีสถานะ checked
          } else {
            $('#AddFormId [name=recuit_stage_status]').prop('checked', false); // ตั้งค่าให้ checkbox ไม่มีสถานะ checked
          }
