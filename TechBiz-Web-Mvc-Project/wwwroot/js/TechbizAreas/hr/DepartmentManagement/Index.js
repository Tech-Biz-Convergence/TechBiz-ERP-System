...
$(document).ready(function () {
  ...
  $(document).on('click', '.ActiveButton', function () {
    ...
    if (button.hasClass('btn-primary')) {
      ...
      urlApi = config.apiUrl.base + '/api/hr/Department/activateCondition/' + id;
      status = "INACTIVE";
  ...
  function ShowMainListDataTable() {
    Page.mainTableControl = $('#tableShowListId').DataTable({
      ...
      columnDefs: [
        ...
        { targets: 1, data: 'dept_name', ordering: true },
        { targets: 2, data: 'emp_name', ordering: true },
        {
          targets: 3,
          data: null,
          orderable: false,
          ...
          render: function (data, type, row) {
            return `<div class="dropdown">
                            ...
                            <a class="dropdown-item ViewButton" href="javascript:void(0);" data-id="${row.dept_id}" 
                                ><i class="bx bx-list-ul me-2"></i> View</a
                              >
                              <a class="dropdown-item EditButton" href="javascript:void(0);" data-id="${row.dept_id}" 
                                ><i class="bx bx-edit-alt me-2"></i> Edit</a
                              >
                              <a class="dropdown-item DeleteButton" href="javascript:void(0);" data-id="${row.dept_id}" 
  ...
  async function AddFromSubmit(formData) {
    ...
    if ($('#AddFormId [name=dept_status]').prop('checked')) {
      formData.dept_status = 'ACTIVE'
    } else {
      formData.dept_status = 'INACTIVE'
    }
      ...
      success: function (response) {
        ...
        if (response.status == true) {
          ...
          if (res.dept_status === "ACTIVE") {
            ...
            $('#AddFormId [name=dept_status]').prop('checked', false); // ตั้งค่าให้ checkbox ไม่มีสถานะ checked
          }
