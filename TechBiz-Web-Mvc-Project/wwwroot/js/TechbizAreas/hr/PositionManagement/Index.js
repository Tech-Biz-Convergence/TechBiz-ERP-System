...
$(document).ready(function () {
  ...
  $(document).on('click', '.DeleteButton', function () {
    MessageBox.ConfirmDeletedMessage("", () => {
      ...
      return $.ajax({
        ...
        url: config.apiUrl.base + '/api/hr/Position/Delete/' + id,
        data: {
          user_name: username
        }
      }).then((response) => {
  ...
  $(document).on('click', '.ActiveButton', function () {
    ...
    if (button.hasClass('btn-primary')) {
      ...
      urlApi = config.apiUrl.base + '/Api/Hr/Position/activateCondition/' + id;
      status = "INACTIVE";
      ...
      columnDefs: [
        {
          ...
          render: function (data, type, row) {
            ...
            // สร้าง HTML ของปุ่ม
            return `<button type="button" class="btn rounded-pill btn-icon ${buttonClass} ActiveButton button-feature " data-id="${row.position_id}">
                              <span class="tf-icons bx bx-power-off"></span>
                        </button>`;
          }
        },
        { targets: 1, data: 'position_name', ordering: true },
        { targets: 2, data: 'dept_name', ordering: true },
        { targets: 3, data: 'level', ordering: true },
        {
          targets: 4,
          data: null,
          orderable: false,
          ...
          render: function (data, type, row) {
            return `<div class="dropdown">
                            ...
                            <a class="dropdown-item ViewButton" href="javascript:void(0);" data-id="${row.position_id}" 
                                ><i class="bx bx-list-ul me-2"></i> View</a
                              >
                              <a class="dropdown-item EditButton" href="javascript:void(0);" data-id="${row.position_id}" 
                                ><i class="bx bx-edit-alt me-2"></i> Edit</a
                              >
                              <a class="dropdown-item DeleteButton" href="javascript:void(0);" data-id="${row.position_id}" 
  ...
  function ShowUploadListDataTable(formData) {
    ...
    $.ajax({
      url: config.apiUrl.base + '/api/hr/position/ImportDataExcelFile',
      headers: { "Authorization": "Bearer " + token },
      type: 'POST',
      data: formData,
      processData: false,
  ...
  async function AddFromSubmit(formData) {
    ...
    if ($('#AddFormId [name=status]').prop('checked')) {
    ...
    if (formData.position_id == '0') {
      ...
      var type = 'POST';
      var url = config.apiUrl.base + '/api/hr/position/addnew';
    } else {
      //update
      formData.update_by = username;
      type = 'PUT';
      url = config.apiUrl.base + '/api/hr/position/update';
    }
    await $.ajax({
      url: url,
      type: type,
  ...
  async function RenderAddNewForm(id) {
    await $.ajax({
      url: config.apiUrl.base + '/api/hr/position/get/' + id,
      type: 'GET',
      contentType: 'application/json; charset=utf-8',
      // data: {
      //     id : id
      ...
      success: function (response) {
        ...
        if (response.status == true) {
          ...
          console.log('dept_name:', res.dept_name); // ตรวจสอบว่ามีค่า dept_name หรือไม่
          console.log('dept_id:', res.dept_id); // ตรวจสอบว่ามีค่า dept_id หรือไม่
         ...
         /* $('#AddFormId [name=dept_id]').val(res.dept_id);*/
          $('#AddFormId [name=level]').val(res.level);
         /* $('#AddFormId [name=dept_name]').val(res.dept_name); */
          ...
          if (res.status === "ACTIVE") {
            $('#AddFormId [name=status]').prop('checked', true); // ตั้งค่าให้ checkbox มีสถานะ checked
          } else {
            $('#AddFormId [name=status]').prop('checked', false); // ตั้งค่าให้ checkbox ไม่มีสถานะ checked
          }
